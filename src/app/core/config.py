...
class AppSettings(BaseSettings):
    APP_NAME: str = config("APP_NAME", default="Budgeting Server")
...
class FirstUserSettings(BaseSettings):
    ...
    ADMIN_PASSWORD: str = config(
        "ADMIN_PASSWORD", default="!Ch4ng3Th1sP4ssW0rd!"
    )
...
class RedisRateLimiterSettings(BaseSettings):
    REDIS_RATE_LIMIT_HOST: str = config(
        "REDIS_RATE_LIMIT_HOST", default="localhost"
    )
...
class DefaultRateLimitSettings(BaseSettings):
    DEFAULT_RATE_LIMIT_LIMIT: int = config(
        "DEFAULT_RATE_LIMIT_LIMIT", default=10
    )
    DEFAULT_RATE_LIMIT_PERIOD: int = config(
        "DEFAULT_RATE_LIMIT_PERIOD", default=3600
    )
